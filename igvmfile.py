...
class cc_blob_sev_info(Structure):
    _pack_ = 1
    _fields_ = [('magic', c_uint32),
                ('version', c_uint16),
                ('reserved', c_uint16),
                ('secrets_phys', c_uint64),
                ('secrets_len', c_uint32),
                ('rsvd1', c_uint32),
                ('cpuid_phys', c_uint64),
                ('cpuid_len', c_uint32),
                ('rsvd2', c_uint32)]

assert sizeof(cc_blob_sev_info) == 40
...
class IGVMFile(VMState):
    ...
    def gen_cpuid_page(self):
        ...
        print("cpuid page %d" % cpuid_page.Count)
        return cpuid_page
...
def load_kernel(kernel, cmdline, ramdisk, vtl, config, sign_key):
    ...
    sev_info.secrets_phys = secrets_page
    sev_info.secrets_len = PGSIZE
    sev_info.cpuid_phys = cpuid_page
