    ...
    def setup_paging(self):
        ...
        if self.regs.efer.LMA == 0:
            ...
            for i in range(PGSIZE // sizeof(PDPTE)):
                ...
                pdpte.pfn = (i << 18) | (CBIT >> 12)
            # PAE is required for 4-level paging
            self.regs.cr4.PAE = 1
            # setup cr3
            self.regs.cr3.value = (pml4addr | CBIT)
...
def load_kernel(kernel, cmdline, ramdisk):
    ...
    kernel_entry = kernel_base + 0x200
    # setup 64-bit architectural env
    state.setup_paging()
    state.setup_gdt()
